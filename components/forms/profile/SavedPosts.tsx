...
import { unsavePost } from "@/lib/service/post.service";
import { getMySavedPosts } from "@/lib/service/user.service";
...
const SavedPosts = () => {
  ...
  const handleUnsave = async () => {
    try {
      ...
      if (token && selectedPostId) {
        await unsavePost(selectedPostId, token);
  ...
  useEffect(() => {
    ...
    const fetchPostsData = async () => {
      try {
        const listPost = await getMySavedPosts(profile._id);
        if (isMounted) {
          setListSavePosts(listPost);
        }
      } catch (error) {
